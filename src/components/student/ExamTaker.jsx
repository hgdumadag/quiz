...
export default function ExamTaker() {
  ...
  const handleBackToExam = useCallback(async () => {
    // Re-open the same exam session with existing answers so the student can revise.
    const unlockedResponses = Object.fromEntries(
      Object.entries(responses).map(([questionId, response]) => [
        questionId,
        { ...response, locked: false, submitted: false },
      ]),
    );
    ...
    dispatch({ type: EXAM_ACTIONS.RESTORE_SESSION, payload: restoredState });
    storageManager.setSessionState(restoredState);
                ...
                {question.type === QUESTION_TYPES.MULTIPLE_CHOICE && (
                  <div className="space-y-2 mb-4">
                    {question.options.map((opt, optIdx) => {
                      ...
                      const isCorrectAnswer = isAssessment && question.correctAnswer === optIdx;
                      ...
                      else if (isUserAnswer) bgClass = 'bg-blue-50 border-blue-300';
                ...
                {question.type === QUESTION_TYPES.TRUE_FALSE && (
                  <div className="flex gap-4 mb-4">
                    {[true, false].map((val) => {
                      ...
                      const isCorrectAnswer = isAssessment && question.correctAnswer === val;
                      ...
                      return (
                        <div key={String(val)} className={`flex-1 p-4 rounded-lg border text-center ${bgClass}`}>
                          ...
                          {isUserAnswer && <span className="block text-xs text-gray-500 mt-1">Your answer</span>}
                          {isCorrectAnswer && <span className="block text-xs text-green-600 mt-1">Correct</span>}
                        </div>
                      );
                    })}
                  </div>
                    ...
                    {isAssessment && question.expectedAnswer && (
                      <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                        ...
                        <p className="text-gray-900 text-sm">
                          <MathText text={question.expectedAnswer} />
                        </p>
                      </div>
                    )}
                  </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="flex items-center justify-between mt-8 pt-4 border-t border-gray-100">
              ...
              <div className="flex items-center gap-3">
                ...
                {/* Submit exam button (last question only) */}
                {isLastQuestion && (
                  <button
