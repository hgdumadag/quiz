...
function examReducer(state, action) {
  switch (action.type) {
    ...
    case EXAM_ACTIONS.CONTINUE_PRACTICE: {
      // Unlock all answers and set status back to IN_PROGRESS for practice mode
      const unlockedResponses = {};
      Object.keys(state.responses).forEach((qId) => {
        unlockedResponses[qId] = {
          ...state.responses[qId],
          locked: false,
          submitted: false,
        };
