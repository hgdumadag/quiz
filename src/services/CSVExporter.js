...
export default class CSVExporter {
  ...
  static exportSummary(results, exams) {
    const examMap = new Map(exams.map((e) => [e.examMetadata?.id || e.id, e]));
    ...
    const rows = results.map((result) => {
      ...
      const examTitle = exam?.examMetadata?.title || exam?.title || result.examId || 'Unknown Exam';
      const totalPoints = result.totalPoints || result.maxScore || result.totalScore || 0;
      const earnedPoints = result.earnedPoints || result.score || 0;
      const pct = result.percentage ?? percentage(earnedPoints, totalPoints);
      const passingScore = result.passingScore ?? exam?.examMetadata?.passingScore ?? exam?.passingScore ?? 70;
  ...
  static exportDetailed(results, exams) {
    ...
    for (const result of results) {
      const exam = examMap.get(result.examId);
      const examTitle = exam?.examMetadata?.title || exam?.title || 'Unknown Exam';
      ...
      for (const question of questions) {
        ...
        const userAnswer = CSVExporter._formatAnswer(grading.userAnswer ?? grading.answer ?? '');
